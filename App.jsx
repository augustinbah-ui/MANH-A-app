...
export default function ManhiaPrototype() {
  ...
  // Supabase place le token de reset dans un fragment du type
  // #access_token=...&type=recovery&... — on détecte ce flux en cherchant
  // "type=recovery" dans le hash, plutôt que de dépendre d'un préfixe custom
  // comme "#reset-password" qui n'apparaît plus dans l'URL générée par Supabase.
  const isRecoveryHash = () => window.location.hash.includes("type=recovery");
  const [isResetRoute, setIsResetRoute] = useState(isRecoveryHash);
  ...
  useEffect(() => {
    const onHashChange = () => {
      ...
      setIsResetRoute(isRecoveryHash());
      const match = window.location.hash.match(/^#pay=(.+)$/);
