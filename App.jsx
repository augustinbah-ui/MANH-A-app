...
async function requestPasswordReset(email) {
  // On fixe l'origine sur le domaine canonique (www) plutôt que d'utiliser
  // window.location.origin : si l'utilisateur est sur manhia.com (sans www) au
  // moment de la demande, le lien pointerait vers manhia.com, ce qui déclenche
  // le redirect 308 de Vercel vers www.manhia.com et perd le token d'auth
  // (#access_token=...) au passage, car les redirects serveur ne transmettent
  // pas les fragments d'URL.
  //
  // Important : on ne met PAS de fragment (#reset-password) ici. Supabase
  // ajoute automatiquement son propre fragment (#access_token=...&type=recovery)
  // à la fin de redirectTo. Si on y met déjà un "#...", ça crée une URL avec
  // DEUX "#" (ex: .../#reset-password#access_token=...), ce qui est invalide :
  // le navigateur ne lit que le premier fragment, donc window.location.hash
  // ne contient jamais le token. Le flux recovery est détecté côté client en
  // lisant "type=recovery" dans le hash reçu (voir isResetRoute plus bas).
  const canonicalOrigin = "https://www.manhia.com";
...
// Version sécurisée de la création de course, utilisée par le flux client standard
// (NewCourseFlow) : le prix et le code promo sont recalculés/revalidés côté serveur
// (Edge Function create-course-secure), pour empêcher un client malveillant d'envoyer
// directement un prix arbitraire ou un statut "payé" non mérité à la base.
async function createCourseSecure({
  clientName, clientPhone, mode, item, stops, needsPurchase, purchaseBudget,
  payerType, recipientName, recipientPhone, country, promoCode,
}) {
  const { data: sessionData } = await supabase.auth.getSession();
  const token = sessionData?.session?.access_token;
  if (!token) throw new Error("Non authentifié");

  const { data, error } = await supabase.functions.invoke("create-course-secure", {
    body: {
      clientName, clientPhone, mode, item, stops, needsPurchase, purchaseBudget,
      payerType, recipientName, recipientPhone, country, promoCode,
    },
  ...
  return { course: mapCourseFromDb(data.course), priceBreakdown: data.priceBreakdown };
}
...
// Valide un fichier avant upload : taille max 8 Mo, type image réel (vérifié via le
// MIME type du fichier, pas juste son extension qui peut être falsifiée).
// Faille corrigée : avant, n'importe quel fichier renommé en .jpg pouvait être envoyé
// sans limite de taille, consommant le quota Supabase ou posant un risque.
const MAX_UPLOAD_SIZE_BYTES = 8 * 1024 * 1024; // 8 Mo
const ALLOWED_IMAGE_TYPES = ["image/jpeg", "image/png", "image/webp", "image/heic", "image/heif"];

function validateUploadFile(file) {
  if (!file) return { valid: false, reason: "Aucun fichier sélectionné." };
  if (file.size > MAX_UPLOAD_SIZE_BYTES) {
    return { valid: false, reason: "L'image est trop volumineuse (8 Mo maximum)." };
  }
  if (file.type && !ALLOWED_IMAGE_TYPES.includes(file.type)) {
    return { valid: false, reason: "Format d'image non pris en charge. Utilisez JPG, PNG ou WebP." };
  }
  return { valid: true };
}
...
// L'accès Admin ne repose plus sur un code partagé en clair (faille corrigée) :
// il exige désormais un vrai compte Supabase Auth avec le champ is_admin à true
// en base, vérifié à la fois côté client (AdminLogin) et côté serveur (policies RLS).
...
/* ---------------- Module Stock (e-commerçants) ---------------- */

async function isStockModuleEnabled() {
  const value = await fetchAppSetting("stock_module_enabled", false);
  return value === true || value === "true";
}

async function setStockModuleEnabled(enabled) {
  await updateAppSetting("stock_module_enabled", enabled);
}

// -- Demande / statut e-commerçant --

async function submitEcommercantRequest(userId, { boutiqueName, contactPhone, contactNote }) {
  const { error } = await supabase
    .from("users")
    .update({
      ecommercant_verification_status: "en_attente",
      ecommercant_boutique_name: boutiqueName,
      ecommercant_contact_phone: contactPhone,
      ecommercant_contact_note: contactNote || null,
    })
    .eq("id", userId);
  if (error) throw error;
}

async function fetchPendingEcommercants() {
  const { data, error } = await supabase
    .from("users")
    .select("*")
    .eq("ecommercant_verification_status", "en_attente")
    .order("created_at", { ascending: true });
  if (error) return [];
  return data;
}

async function fetchAllEcommercants() {
  const { data, error } = await supabase
    .from("users")
    .select("*")
    .in("ecommercant_verification_status", ["en_attente", "verifie", "refuse"])
    .order("created_at", { ascending: false });
  if (error) return [];
  return data;
}

async function setEcommercantVerificationStatus(userId, status) {
  const patch = { ecommercant_verification_status: status };
  if (status === "verifie") patch.is_ecommercant = true;
  const { error } = await supabase.from("users").update(patch).eq("id", userId);
  if (error) throw error;
}

async function updateEcommercantPaymentMode(userId, mode) {
  const { error } = await supabase.from("users").update({ ecommercant_payment_mode: mode }).eq("id", userId);
  if (error) throw error;
}

// -- Tarification stockage --

async function fetchPricingStockage() {
  const { data, error } = await supabase.from("pricing_stockage").select("*").order("frais_fixe_mois", { ascending: true });
  if (error) return [];
  return data;
}

async function updatePricingStockage(categorie, { fraisFixeMois, fraisPourcentValeur, exempleProduits }) {
  const { error } = await supabase
    .from("pricing_stockage")
    .update({
      frais_fixe_mois: fraisFixeMois,
      frais_pourcent_valeur: fraisPourcentValeur,
      exemple_produits: exempleProduits,
      updated_at: new Date().toISOString(),
    })
...
async function fetchEntrepotConfig(country = "benin") {
  const { data, error } = await supabase.from("entrepot_config").select("*").eq("id", "main").maybeSingle();
  if (error || !data) return null;
  return data;
}

async function updateEntrepotConfig({ label, lat, lng, country }) {
  const { error } = await supabase
    .from("entrepot_config")
    .update({ label, lat, lng, country, updated_at: new Date().toISOString() })
    .eq("id", "main");
  if (error) throw error;
}
...
function mapStockProduitFromDb(row) {
  return {
    id: row.id,
    ecommercantId: row.ecommercant_id,
    nomProduit: row.nom_produit,
    reference: row.reference,
    categorie: row.categorie,
    valeurUnitaire: Number(row.valeur_unitaire),
    quantiteRecue: row.quantite_recue,
    quantiteVendue: row.quantite_vendue,
    quantiteDisponible: row.quantite_recue - row.quantite_vendue,
    statut: row.statut,
    dateReception: row.date_reception ? new Date(row.date_reception).getTime() : null,
    dateEpuisement: row.date_epuisement ? new Date(row.date_epuisement).getTime() : null,
    createdAt: row.created_at ? new Date(row.created_at).getTime() : null,
  };
}

async function fetchStockProduitsForEcommercant(ecommercantId) {
  const { data, error } = await supabase
    .from("stock_produits")
    .select("*")
    .eq("ecommercant_id", ecommercantId)
    .order("created_at", { ascending: false });
  if (error) return [];
  return data.map(mapStockProduitFromDb);
}

async function fetchAllStockProduits() {
  const { data, error } = await supabase
    .from("stock_produits")
    .select("*, users:ecommercant_id (name, ecommercant_boutique_name)")
    .order("created_at", { ascending: false });
  if (error) return [];
  return data.map((row) => ({
    ...mapStockProduitFromDb(row),
    ecommercantName: row.users?.ecommercant_boutique_name || row.users?.name || "—",
  }));
}

async function createStockProduit({ ecommercantId, nomProduit, reference, categorie, valeurUnitaire, quantiteRecue }) {
  const { data, error } = await supabase
    .from("stock_produits")
    .insert({
      ecommercant_id: ecommercantId,
    ...
    .single();
  if (error) throw error;
  return mapStockProduitFromDb(data);
}

async function updateStockProduit(produitId, patch) {
  const dbPatch = {};
  if (patch.nomProduit !== undefined) dbPatch.nom_produit = patch.nomProduit;
  ...
  if (patch.valeurUnitaire !== undefined) dbPatch.valeur_unitaire = patch.valeurUnitaire;
  if (patch.quantiteRecue !== undefined) dbPatch.quantite_recue = patch.quantiteRecue;
  if (patch.statut !== undefined) dbPatch.statut = patch.statut;
  if (patch.dateEpuisement !== undefined) dbPatch.date_epuisement = patch.dateEpuisement;
  const { error } = await supabase.from("stock_produits").update(dbPatch).eq("id", produitId);
  if (error) throw error;
}

async function deleteStockProduit(produitId) {
  const { error } = await supabase.from("stock_produits").delete().eq("id", produitId);
  if (error) throw error;
}

// Décrémente la quantité vendue d'un produit lorsqu'une commande stock est passée.
// Si le stock disponible tombe à 0, marque le produit "epuise" et enregistre la date
// (nécessaire au calcul au prorata des frais de stockage).
async function decrementStockOnOrder(produitId, quantiteCommandee) {
  const { data: produit, error: fetchErr } = await supabase.from("stock_produits").select("*").eq("id", produitId).single();
  if (fetchErr) throw fetchErr;
  const nouvelleQuantiteVendue = produit.quantite_vendue + quantiteCommandee;
  const disponible = produit.quantite_recue - nouvelleQuantiteVendue;
  const patch = { quantite_vendue: nouvelleQuantiteVendue };
  if (disponible <= 0) {
    patch.statut = "epuise";
    patch.date_epuisement = new Date().toISOString();
  }
  const { error } = await supabase.from("stock_produits").update(patch).eq("id", produitId);
  if (error) throw error;
}

// -- Commandes stock (produit -> course) --

function mapStockCommandeFromDb(row) {
  return {
    id: row.id,
    ecommercantId: row.ecommercant_id,
    ...
    clientFinalNom: row.client_final_nom,
    clientFinalPhone: row.client_final_phone,
...
async function fetchStockCommandesForEcommercant(ecommercantId) {
  const { data, error } = await supabase
    .from("stock_commandes")
    .select("*, stock_produits:produit_id (nom_produit)")
    .eq("ecommercant_id", ecommercantId)
    .order("created_at", { ascending: false });
  if (error) return [];
  return data.map((row) => ({
    ...mapStockCommandeFromDb(row),
    produitNom: row.stock_produits?.nom_produit || "—",
  }));
}

// Crée une commande stock : décrémente le produit, crée la course de livraison réelle
// (départ = entrepôt fixe, arrivée = adresse saisie par l'e-commerçant), et lie les deux.
async function createStockCommande({
  ecommercant, // { id, name, phone, country }
  produit,     // objet produit (mapStockProduitFromDb)
  quantite,
  destination, // { label, lat, lng }
  clientFinalNom,
  clientFinalPhone,
  payerType,   // "client_final" | "ecommercant"
}) {
  const entrepot = await fetchEntrepotConfig(ecommercant.country);
  if (!entrepot || entrepot.lat == null || entrepot.lng == null) {
    throw new Error("L'adresse de l'entrepôt n'est pas encore configurée par l'admin.");
  }
  const stops = [
    { label: entrepot.label, lat: Number(entrepot.lat), lng: Number(entrepot.lng) },
    { label: destination.label, lat: destination.lat, lng: destination.lng },
  ];
  const { price, distanceKm } = computePrice(stops, "tournee");
  const valeurMarchandise = produit.valeurUnitaire * quantite;

  const course = await createCourse({
    clientId: ecommercant.id,
    clientName: payerType === "client_final" ? clientFinalNom : ecommercant.name,
    clientPhone: payerType === "client_final" ? clientFinalPhone : ecommercant.phone,
    mode: "tournee",
    item: `Stock : ${produit.nomProduit} x${quantite}`,
    stops,
    price,
    distanceKm,
    needsPurchase: false,
    payerType: payerType === "client_final" ? "destinataire" : "client",
    recipientName: payerType === "client_final" ? clientFinalNom : null,
    recipientPhone: payerType === "client_final" ? clientFinalPhone : null,
    country: ecommercant.country || "benin",
    history: [
      { label: `Commande stock créée par ${ecommercant.name} (${produit.nomProduit} x${quantite}, valeur ${valeurMarchandise.toLocaleString()} FCFA)`, at: Date.now() },
    ],
  });
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      client_final_phone: clientFinalPhone,
  ...
  await decrementStockOnOrder(produit.id, quantite);

  return { commande: mapStockCommandeFromDb(data), course };
}

// -- Frais de stockage mensuels --

function moisKey(date = new Date()) {
  const y = date.getFullYear();
  const m = String(date.getMonth() + 1).padStart(2, "0");
  return `${y}-${m}`;
}

// Calcule le nombre de jours de stockage d'un produit sur le mois donné, au prorata :
// du 1er jour du mois (ou date de réception si plus tardive) jusqu'au dernier jour du
// mois (ou date d'épuisement si le produit s'est vidé avant la fin du mois).
function joursStockageDansLeMois(produit, moisStr) {
  const [y, m] = moisStr.split("-").map(Number);
  const debutMois = new Date(y, m - 1, 1);
  const finMois = new Date(y, m, 0, 23, 59, 59);
  const reception = produit.dateReception ? new Date(produit.dateReception) : debutMois;
  const epuisement = produit.dateEpuisement ? new Date(produit.dateEpuisement) : null;

  const debut = reception > debutMois ? reception : debutMois;
  const fin = epuisement && epuisement < finMois ? epuisement : finMois;
...
// Calcule le montant dû pour un e-commerçant sur un mois donné, à partir de son stock
// et de la grille tarifaire, au prorata des jours de stockage effectifs.
function calculerFraisStockageMensuel(produits, pricingStockage, moisStr) {
  const [y, m] = moisStr.split("-").map(Number);
  const joursDansLeMois = new Date(y, m, 0).getDate();
  const pricingParCategorie = {};
  pricingStockage.forEach((p) => { pricingParCategorie[p.categorie] = p; });

  let total = 0;
  const details = [];
  produits.forEach((produit) => {
    const pricing = pricingParCategorie[produit.categorie];
    if (!pricing) return;
    const jours = joursStockageDansLeMois(produit, moisStr);
    if (jours <= 0) return;
    const fraisFixeProrata = (Number(pricing.frais_fixe_mois) / joursDansLeMois) * jours;
    const fraisValeur = produit.valeurUnitaire * (produit.quantiteRecue - produit.quantiteVendue) * Number(pricing.frais_pourcent_valeur);
    const montant = Math.round(fraisFixeProrata + fraisValeur);
    total += montant;
    details.push({ produit, jours, montant });
  });
  return { total, details };
}

async function fetchFraisStockage(ecommercantId, moisStr) {
  const { data, error } = await supabase
    .from("stock_frais_mensuel")
    .select("*")
    .eq("ecommercant_id", ecommercantId)
    .eq("mois", moisStr)
    .maybeSingle();
...
async function fetchAllFraisStockageForMonth(moisStr) {
  const { data, error } = await supabase
    .from("stock_frais_mensuel")
    .select("*, users:ecommercant_id (name, ecommercant_boutique_name, phone)")
    .eq("mois", moisStr)
    .order("created_at", { ascending: false });
  if (error) return [];
  return data.map((row) => ({
    ...row,
    ecommercantName: row.users?.ecommercant_boutique_name || row.users?.name || "—",
    ecommercantPhone: row.users?.phone || "—",
  }));
}

// Enregistre / met à jour le montant dû pour un e-commerçant sur un mois (upsert).
async function upsertFraisStockage(ecommercantId, moisStr, { montantDu, acompteVerse, paye, note }) {
  const { error } = await supabase
    .from("stock_frais_mensuel")
    .upsert(
      {
      ...
      { onConflict: "ecommercant_id,mois" }
    );
  if (error) throw error;
}

async function markFraisStockagePaye(id, note) {
  const { error } = await supabase.from("stock_frais_mensuel").update({ paye: true, note: note || null }).eq("id", id);
  if (error) throw error;
}
...
function AuthScreen({ onAuth }) {
  ...
  const submit = async () => {
    ...
    // Validation de format, en plus de la simple présence des champs (faille corrigée :
    // avant, un téléphone ou un email mal formé pouvait être accepté sans contrôle).
    if (mode === "signup") {
      ...
      if (phoneDigits.length < 8 || phoneDigits.length > 12) {
        setError("Indiquez un numéro de téléphone valide (8 à 12 chiffres).");
        return;
      }
...
function NewCourseFlow({ user, onCreated, onCancel, serviceType = "envoyer" }) {
  ...
  const confirmBooking = async () => {
    ...
    setPosting(true);
    try {
      // Le prix, le calcul de distance et la validation du code promo sont désormais
      // entièrement recalculés côté serveur (Edge Function) — ce que le client envoie
      // ici (promoCode) n'est qu'une suggestion, jamais une valeur de confiance finale.
      const { course: created } = await createCourseSecure({
      ...
      fetchOnlineLivreurIds().then((ids) => {
        if (ids.length > 0) {
          sendPushNotification(ids, "Nouvelle course disponible", `${item} · ${created.price.toLocaleString()} FCFA`, "#");
        }
      });
    } catch (e) {
...
function ClientApp({ user, onLogout }) {
  ...
  const [stockModuleEnabled, setStockModuleEnabledState] = useState(false);
  const [ecommercantStatus, setEcommercantStatus] = useState(user.ecommercantVerificationStatus || "non_demande");
  const [showEcommercantRequest, setShowEcommercantRequest] = useState(false);
  const [showEcommercantApp, setShowEcommercantApp] = useState(false);

  useEffect(() => {
    fetchActivePromotions().then(setAds);
    isStockModuleEnabled().then(setStockModuleEnabledState);
  }, []);

  useEffect(() => {
    if (!stockModuleEnabled || ecommercantStatus !== "en_attente") return;
    const interval = setInterval(async () => {
      const fresh = await findUserById(user.id);
      if (fresh && fresh.ecommercant_verification_status !== ecommercantStatus) {
        setEcommercantStatus(fresh.ecommercant_verification_status);
      }
    }, 5000);
    return () => clearInterval(interval);
  }, [stockModuleEnabled, ecommercantStatus, user.id]);
  ...
  if (showEcommercantRequest) {
    return (
      <EcommercantRequestScreen
        user={user}
        status={ecommercantStatus === "en_attente" ? "en_attente" : "form"}
        onSubmitted={() => { setEcommercantStatus("en_attente"); }}
        onClose={() => setShowEcommercantRequest(false)}
      />
    );
  }

  if (showEcommercantApp) {
    return (
      <EcommercantApp
        user={user}
        onClose={() => setShowEcommercantApp(false)}
      />
    );
  }
          ...
          <div className="pb-6">
            ...
            <div className="px-5 mt-4">
              ...
              {stockModuleEnabled && (
                <button
                  onClick={() => {
                    if (ecommercantStatus === "verifie") setShowEcommercantApp(true);
                    else setShowEcommercantRequest(true);
                  }}
                  className="w-full rounded-2xl p-4 flex items-center gap-3 text-left mt-3"
                  style={{ background: C.white, border: `1px solid ${C.line}` }}
                >
                  <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0" style={{ background: `${C.lagoon}1A` }}>
                    <ShoppingBag size={18} color={C.lagoon} />
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>
                      {ecommercantStatus === "verifie" ? "Mon stock" : "Passer à e-commerçant"}
                    </p>
                    <p className="text-xs" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
                      {ecommercantStatus === "verifie"
                        ? "Gérez votre stock et vos commandes"
                        : ecommercantStatus === "en_attente"
                        ? "Demande en cours de validation"
                        : "Stockez vos produits et créez vos livraisons"}
                    </p>
                  </div>
                  <ChevronRight size={16} color={C.inkSoft} />
                </button>
...
/* ===========================================================
   MODULE STOCK — ÉCRANS E-COMMERÇANT
=========================================================== */

// Formulaire de demande pour devenir e-commerçant, et écrans d'attente/refus,
// sur le même modèle que LivreurVerificationScreen.
// Écran affiché pendant qu'une demande e-commerçant est en attente de validation par l'admin.
function EcommercantPendingScreen({ onClose }) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: C.sand }}>
      <Clock size={36} color={C.zem} className="mb-4" />
      <h1 className="text-lg font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>Demande en cours</h1>
      <p className="text-xs mt-2 leading-snug max-w-xs" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
        L'équipe Manhïa examine votre demande pour devenir e-commerçant. Vous serez averti dès validation.
      </p>
      <button onClick={onClose} className="mt-6 text-xs font-semibold underline" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
        Retour
      </button>
    </div>
  );
...
function EcommercantRequestScreen({ user, status, onSubmitted, onClose }) {
  ...
  const submit = async () => {
    if (!boutiqueName.trim() || !contactPhone.trim()) {
      setError("Le nom de la boutique et un numéro de contact sont nécessaires.");
      return;
    }
    setError("");
    setSubmitting(true);
    try {
      await submitEcommercantRequest(user.id, { boutiqueName, contactPhone, contactNote });
      notifyAdminByEmail(
        "Nouvelle demande e-commerçant",
        `${user.name} (${user.phone}) souhaite devenir e-commerçant. Boutique : ${boutiqueName}.`
      );
      onSubmitted();
    } catch {
      setError("Erreur lors de l'envoi. Réessayez.");
  ...
  if (status === "en_attente") {
    return <EcommercantPendingScreen onClose={onClose} />;
  }

  return (
    <div className="min-h-screen px-6 pt-12 pb-8 flex flex-col" style={{ background: C.sand }}>
      <div className="text-center mb-6">
        <ShoppingBag size={32} color={C.lagoon} className="mx-auto mb-3" />
        <h1 className="text-lg font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>Devenir e-commerçant</h1>
        <p className="text-xs mt-2 leading-snug" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
          Stockez vos produits chez Manhïa et créez vos commandes de livraison directement depuis l'application.
        </p>
      </div>

      {status === "refuse" && (
        <div className="rounded-xl p-3 mb-4" style={{ background: `${C.danger}14`, border: `1px solid ${C.danger}` }}>
          <p className="text-xs" style={{ color: C.danger, fontFamily: FONT_BODY }}>
      ...
      <div className="space-y-4">
        <TextField label="Nom de la boutique" value={boutiqueName} onChange={setBoutiqueName} placeholder="Ex : Chez Awa Cosmétiques" icon={ShoppingBag} />
        <TextField label="Numéro de contact" value={contactPhone} onChange={setContactPhone} placeholder="Numéro joignable" icon={Phone} />
        <TextField label="Note (optionnel)" value={contactNote} onChange={setContactNote} placeholder="Canal préféré, disponibilité..." />
      </div>
      ...
      <button onClick={onClose} className="mt-4 text-xs font-semibold underline mx-auto" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
        Annuler
      </button>
    </div>
  );
...
function StockProduitCard({ produit }) {
  const disponible = produit.quantiteDisponible;
  const tone = produit.statut === "epuise" ? "danger" : produit.statut === "a_verifier" ? "zem" : "lagoon";
  const label = produit.statut === "epuise" ? "Épuisé" : produit.statut === "a_verifier" ? "À vérifier" : "Disponible";
  return (
    <div className="rounded-2xl p-4" style={{ background: C.white, border: `1px solid ${C.line}` }}>
      <div className="flex items-center justify-between mb-2">
        <Tag tone={tone}>{label}</Tag>
        <span className="text-[10px] uppercase" style={{ color: C.inkSoft, fontFamily: FONT_DISPLAY }}>{produit.categorie}</span>
      </div>
      <p className="text-sm font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>{produit.nomProduit}</p>
      {produit.reference && (
        <p className="text-[11px] mt-0.5" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>Réf. {produit.reference}</p>
      )}
      <div className="flex items-center justify-between mt-3">
        <p className="text-xs" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
          {disponible} dispo · {produit.valeurUnitaire.toLocaleString()} FCFA/unité
        </p>
        <p className="text-xs font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>
          {(disponible * produit.valeurUnitaire).toLocaleString()} FCFA
        </p>
      </div>
    </div>
  );
...
// Modal de création de commande stock : l'e-commerçant choisit un produit disponible,
// une quantité, saisit l'adresse finale via le picker existant, et les infos du client final.
function NewStockCommandeFlow({ user, produits, onClose, onDone }) {
  ...
  const [payerType, setPayerType] = useState("client_final");
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState("");

  const produit = produits.find((p) => p.id === produitId) || produits[0];
  const qte = Math.max(1, parseInt(quantite, 10) || 1);
  const qteValide = produit ? Math.min(qte, produit.quantiteDisponible) : 0;

  const produitLabels = produits.map((p) => `${p.nomProduit} (${p.quantiteDisponible} dispo)`);
  const selectedLabel = produit ? `${produit.nomProduit} (${produit.quantiteDisponible} dispo)` : "";

  const submit = async () => {
    if (!produit) { setError("Choisissez un produit."); return; }
    if (qteValide < 1) { setError("Quantité invalide ou stock insuffisant."); return; }
    if (!destination) { setError("Choisissez l'adresse de livraison."); return; }
    if (!clientFinalNom.trim() || !clientFinalPhone.trim()) {
      setError("Le nom et le numéro du client final sont nécessaires.");
      return;
    }
    setError("");
    setSubmitting(true);
    try {
      await createStockCommande({
        ecommercant: { id: user.id, name: user.name, phone: user.phone, country: user.country },
        produit,
        quantite: qteValide,
        destination,
        clientFinalNom,
      ...
      setError(e.message || "Erreur lors de la création de la commande. Réessayez.");
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end" style={{ background: "rgba(34,32,27,0.5)" }}>
      <div className="w-full max-h-[92vh] overflow-y-auto rounded-t-3xl p-5 pb-8" style={{ background: C.sand }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>Nouvelle commande</h2>
          <button onClick={onClose}><X size={18} color={C.inkSoft} /></button>
        </div>

        {produits.length === 0 ? (
          <EmptyState icon={Package} title="Aucun produit disponible" sub="Contactez Manhïa pour enregistrer votre stock avant de créer une commande." />
        ) : (
          <div className="space-y-4">
            <Select
              label="Produit"
              value={selectedLabel}
              onChange={(label) => {
                const idx = produitLabels.indexOf(label);
                if (idx >= 0) setProduitId(produits[idx].id);
              }}
            ...
            <div>
              ...
              <input
                type="number"
                min="1"
                max={produit?.quantiteDisponible || 1}
                value={quantite}
            ...
            >
              ...
              <p className="text-sm truncate" style={{ color: destination ? C.ink : C.inkSoft, fontFamily: FONT_BODY }}>
                {destination ? destination.label : "Toucher pour choisir sur la carte"}
              </p>
            </button>

            <TextField label="Nom du client final" value={clientFinalNom} onChange={setClientFinalNom} placeholder="Nom complet" icon={User} />
            <TextField label="Téléphone du client final" value={clientFinalPhone} onChange={setClientFinalPhone} placeholder="Numéro joignable" icon={Phone} />

            <div>
              <label className="text-[11px] font-bold uppercase tracking-[0.1em] mb-2 block" style={{ color: C.inkSoft, fontFamily: FONT_DISPLAY }}>
                Qui paie la livraison ?
              </label>
              <div className="flex gap-2">
                <button
                  onClick={() => setPayerType("client_final")}
                  className="flex-1 py-2.5 rounded-xl text-xs font-bold"
                  style={{ background: payerType === "client_final" ? C.lagoon : C.white, color: payerType === "client_final" ? C.white : C.ink, border: `1px solid ${C.line}`, fontFamily: FONT_DISPLAY }}
                >
                  Le client final
                </button>
                <button
                  onClick={() => setPayerType("ecommercant")}
                  className="flex-1 py-2.5 rounded-xl text-xs font-bold"
                  style={{ background: payerType === "ecommercant" ? C.lagoon : C.white, color: payerType === "ecommercant" ? C.white : C.ink, border: `1px solid ${C.line}`, fontFamily: FONT_DISPLAY }}
                >
                  Moi (boutique)
                </button>
              </div>
            ...
            {produit && destination && (
              <div className="rounded-xl p-3" style={{ background: C.lagoonDeep }}>
                <p className="text-[11px]" style={{ color: "#BFE0D6", fontFamily: FONT_BODY }}>Valeur marchandise</p>
                <p className="text-sm font-bold" style={{ color: C.white, fontFamily: FONT_DISPLAY }}>
                  {(produit.valeurUnitaire * qteValide).toLocaleString()} FCFA
                </p>
              </div>
            )}
      ...
      {pickerOpen && (
        <LocationPickerModal
          ...
          onConfirm={(place) => { setDestination(place); setPickerOpen(false); }}
        />
      )}
    </div>
  );
...
function StockCommandeCard({ commande }) {
  const toneMap = {
    en_attente: "zem",
    collecte_impossible: "danger",
    en_cours: "lagoon",
    livree: "lagoon",
    annulee: "danger",
  };
  const labelMap = {
    ...
    collecte_impossible: "Collecte impossible",
    en_cours: "En cours",
    livree: "Livrée",
    annulee: "Annulée",
  };
  return (
    <div className="rounded-2xl p-4" style={{ background: C.white, border: `1px solid ${C.line}` }}>
      <div className="flex items-center justify-between mb-2">
        <Tag tone={toneMap[commande.statut] || "lagoon"}>{labelMap[commande.statut] || commande.statut}</Tag>
        <span className="text-[10px]" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
          {new Date(commande.createdAt).toLocaleDateString("fr-FR")}
        </span>
      </div>
      <p className="text-sm font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>{commande.produitNom} x{commande.quantite}</p>
      <p className="text-xs mt-1" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
        Client : {commande.clientFinalNom} · {commande.clientFinalPhone}
      </p>
    </div>
  );
}

// Dashboard principal de l'e-commerçant : accessible depuis le profil client une fois vérifié.
// Affiche le stock disponible, l'historique des commandes, et permet d'en créer une nouvelle.
function EcommercantApp({ user, onClose }) {
  ...
  const load = useCallback(async () => {
    const [p, c] = await Promise.all([
      fetchStockProduitsForEcommercant(user.id),
      fetchStockCommandesForEcommercant(user.id),
    ]);
    setProduits(p);
    setCommandes(c);
    setLoading(false);
  ...
  const produitsDisponibles = produits.filter((p) => p.statut === "disponible" && p.quantiteDisponible > 0);
  const valeurTotaleStock = produits.reduce((s, p) => s + p.quantiteDisponible * p.valeurUnitaire, 0);

  return (
    <div className="min-h-screen" style={{ background: C.sand }}>
      <TopBar title="Mon stock" onBack={onClose} />

      <div className="px-5 mt-4">
        <div className="rounded-2xl p-4" style={{ background: C.lagoonDeep }}>
          <p className="text-[11px]" style={{ color: "#BFE0D6", fontFamily: FONT_BODY }}>Valeur du stock disponible</p>
          <p className="text-2xl font-bold mt-1" style={{ color: C.white, fontFamily: FONT_DISPLAY }}>
      ...
      <div className="flex px-5 mt-4 gap-2">
        <button
          ...
          className="flex-1 py-2.5 rounded-full text-xs font-bold"
          style={{ background: tab === "stock" ? C.ink : C.white, color: tab === "stock" ? C.white : C.ink, border: `1px solid ${C.line}`, fontFamily: FONT_DISPLAY }}
        >
          Mon stock
        </button>
        <button
          ...
          className="flex-1 py-2.5 rounded-full text-xs font-bold"
          style={{ background: tab === "commandes" ? C.ink : C.white, color: tab === "commandes" ? C.white : C.ink, border: `1px solid ${C.line}`, fontFamily: FONT_DISPLAY }}
        >
          Commandes
        </button>
      </div>
      ...
      <div className="px-5 mt-4 pb-28 space-y-3">
        {loading ? (
          ...
          produits.length === 0 ? (
            <EmptyState icon={Package} title="Aucun produit enregistré" sub="Contactez Manhïa pour enregistrer votre stock initial." />
          ) : (
            produits.map((p) => <StockProduitCard key={p.id} produit={p} />)
          )
        ) : commandes.length === 0 ? (
          <EmptyState icon={ListOrdered} title="Aucune commande" sub="Vos commandes créées depuis votre stock apparaîtront ici." />
        ) : (
          commandes.map((c) => <StockCommandeCard key={c.id} commande={c} />)
        )}
      </div>

      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-4" style={{ background: C.sand, borderTop: `1px solid ${C.line}` }}>
        <PrimaryButton onClick={() => setCreating(true)} disabled={produitsDisponibles.length === 0}>
          Créer une commande
        </PrimaryButton>
      </div>

      {creating && (
        <NewStockCommandeFlow
          user={user}
...
function AdminLogin({ onSuccess }) {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center px-6" style={{ background: C.ink }}>
      ...
      <div className="space-y-3">
        <TextField label="Email" value={email} onChange={setEmail} placeholder="admin@manhia.com" icon={User} />
        <TextField label="Mot de passe" value={password} onChange={setPassword} type="password" placeholder="Votre mot de passe" />
      </div>
      {error && <p className="text-xs mt-3 font-semibold" style={{ color: "#F08A6C", fontFamily: FONT_BODY }}>{error}</p>}
      <div className="mt-5">
        <button onClick={submit} disabled={loading} className="w-full py-3.5 rounded-2xl text-sm font-bold" style={{ background: C.zem, color: C.ink, fontFamily: FONT_DISPLAY, opacity: loading ? 0.7 : 1 }}>
...
// La session admin repose désormais entièrement sur la session Supabase Auth
// (le même mécanisme que pour les clients/livreurs), plus de code partagé en clair
// ni de session localStorage séparée. isAdminAuthenticated vérifie le champ is_admin
// du profil courant, déjà chargé dans le state principal de l'app.
function isAdminAuthenticated(user) {
...
/* ===========================================================
   MODULE STOCK — PANNEAU ADMIN
=========================================================== */

function AddStockProduitModal({ ecommercants, onClose, onDone }) {
  const [ecommercantId, setEcommercantId] = useState(ecommercants[0]?.id || "");
  const [nomProduit, setNomProduit] = useState("");
  const [reference, setReference] = useState("");
  const [categorie, setCategorie] = useState("petit");
  const [valeurUnitaire, setValeurUnitaire] = useState("");
  const [quantiteRecue, setQuantiteRecue] = useState("1");
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState("");

  const ecommercantLabels = ecommercants.map((e) => e.ecommercant_boutique_name || e.name);
  const selectedLabel = (ecommercants.find((e) => e.id === ecommercantId)?.ecommercant_boutique_name) || (ecommercants.find((e) => e.id === ecommercantId)?.name) || "";

  const submit = async () => {
    if (!ecommercantId || !nomProduit.trim() || !valeurUnitaire || !quantiteRecue) {
    ...
    setSubmitting(true);
    try {
      await createStockProduit({
        ecommercantId,
        nomProduit,
        reference,
        categorie,
        valeurUnitaire: Number(valeurUnitaire),
        quantiteRecue: Number(quantiteRecue),
      });
      onDone();
    } catch {
      setError("Erreur lors de l'enregistrement. Réessayez.");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end" style={{ background: "rgba(34,32,27,0.5)" }}>
      <div className="w-full max-h-[92vh] overflow-y-auto rounded-t-3xl p-5 pb-8" style={{ background: C.sand }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>Ajouter un produit</h2>
          <button onClick={onClose}><X size={18} color={C.inkSoft} /></button>
        </div>
        {ecommercants.length === 0 ? (
          <p className="text-xs" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>Aucun e-commerçant vérifié pour l'instant.</p>
        ) : (
          <div className="space-y-4">
            <Select
              label="E-commerçant"
              value={selectedLabel}
              onChange={(label) => {
                const idx = ecommercantLabels.indexOf(label);
                if (idx >= 0) setEcommercantId(ecommercants[idx].id);
              }}
              options={ecommercantLabels}
            />
            <TextField label="Nom du produit" value={nomProduit} onChange={setNomProduit} placeholder="Ex : Crème hydratante 200ml" />
            <TextField label="Référence (optionnel)" value={reference} onChange={setReference} placeholder="SKU / réf. interne" />
            <Select label="Catégorie" value={categorie} onChange={setCategorie} options={["petit", "moyen", "grand"]} />
            <TextField label="Valeur unitaire (FCFA)" value={valeurUnitaire} onChange={setValeurUnitaire} type="text" placeholder="Ex : 5000" />
            <TextField label="Quantité reçue" value={quantiteRecue} onChange={setQuantiteRecue} type="text" placeholder="Ex : 20" />
            {error && <p className="text-xs" style={{ color: C.danger, fontFamily: FONT_BODY }}>{error}</p>}
            <PrimaryButton onClick={submit} disabled={submitting}>
...
function EditStockProduitModal({ produit, onClose, onDone }) {
  ...
  const [valeurUnitaire, setValeurUnitaire] = useState(String(produit.valeurUnitaire));
  const [quantiteRecue, setQuantiteRecue] = useState(String(produit.quantiteRecue));
  const [statut, setStatut] = useState(produit.statut);
  const [submitting, setSubmitting] = useState(false);

  const submit = async () => {
    setSubmitting(true);
    try {
      await updateStockProduit(produit.id, {
        nomProduit,
        reference,
        categorie,
        valeurUnitaire: Number(valeurUnitaire),
  ...
  const remove = async () => {
    ...
    setSubmitting(true);
    try {
      await deleteStockProduit(produit.id);
      onDone();
    } catch {
      setSubmitting(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end" style={{ background: "rgba(34,32,27,0.5)" }}>
      <div className="w-full max-h-[92vh] overflow-y-auto rounded-t-3xl p-5 pb-8" style={{ background: C.sand }}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>Modifier le produit</h2>
          <button onClick={onClose}><X size={18} color={C.inkSoft} /></button>
        </div>
        <div className="space-y-4">
          <TextField label="Nom du produit" value={nomProduit} onChange={setNomProduit} />
          <TextField label="Référence" value={reference} onChange={setReference} />
          <Select label="Catégorie" value={categorie} onChange={setCategorie} options={["petit", "moyen", "grand"]} />
          <TextField label="Valeur unitaire (FCFA)" value={valeurUnitaire} onChange={setValeurUnitaire} />
          <TextField label="Quantité reçue (total)" value={quantiteRecue} onChange={setQuantiteRecue} />
          <Select label="Statut" value={statut} onChange={setStatut} options={["disponible", "a_verifier", "epuise"]} />
          <PrimaryButton onClick={submit} disabled={submitting}>
...
function EcommercantsVerificationPanel({ ecommercants, onValidate, onRefuse, onManageProduits }) {
  const pending = ecommercants.filter((e) => e.ecommercant_verification_status === "en_attente");
  const others = ecommercants.filter((e) => e.ecommercant_verification_status !== "en_attente");
    ...
    <div className="space-y-3">
      {pending.length > 0 && (
        <>
          ...
          {pending.map((e) => (
            <div key={e.id} className="rounded-2xl p-4" style={{ background: C.white, border: `1px solid ${C.zem}` }}>
              <div className="flex items-center justify-between mb-2">
                <Tag tone="zem">En attente</Tag>
                <span className="text-[10px]" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>{e.phone}</span>
              </div>
              <p className="text-sm font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>{e.name}</p>
              <p className="text-xs mt-1" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
                Boutique : {e.ecommercant_boutique_name} · Contact : {e.ecommercant_contact_phone}
              </p>
              {e.ecommercant_contact_note && (
                <p className="text-xs mt-1" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>Note : {e.ecommercant_contact_note}</p>
              )}
              <div className="flex gap-2 mt-3">
                <button onClick={() => onValidate(e)} className="flex-1 py-2 rounded-xl text-xs font-bold" style={{ background: C.lagoon, color: C.white, fontFamily: FONT_DISPLAY }}>
                  Valider
                </button>
                <button onClick={() => onRefuse(e)} className="flex-1 py-2 rounded-xl text-xs font-bold" style={{ background: C.white, border: `1px solid ${C.danger}`, color: C.danger, fontFamily: FONT_DISPLAY }}>
                  Refuser
                </button>
              </div>
            </div>
      ...
      <p className="text-xs font-bold uppercase tracking-[0.14em] mt-2" style={{ color: C.inkSoft, fontFamily: FONT_DISPLAY }}>
        E-commerçants
      </p>
      {others.length === 0 ? (
        <EmptyState icon={ShoppingBag} title="Aucun e-commerçant" sub="Les e-commerçants vérifiés apparaîtront ici." />
      ) : (
        others.map((e) => (
          <div key={e.id} className="rounded-2xl p-4" style={{ background: C.white, border: `1px solid ${C.line}` }}>
            <div className="flex items-center justify-between mb-2">
              <Tag tone={e.ecommercant_verification_status === "verifie" ? "lagoon" : "danger"}>
                {e.ecommercant_verification_status === "verifie" ? "Vérifié" : "Refusé"}
              </Tag>
              <span className="text-[10px]" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>{e.phone}</span>
            </div>
            <p className="text-sm font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>{e.ecommercant_boutique_name || e.name}</p>
            <p className="text-xs mt-1" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
              Mode paiement : {e.ecommercant_payment_mode === "acompte_50" ? "Acompte 50%" : "Avance 100%"}
            </p>
            {e.ecommercant_verification_status === "verifie" && (
              <button
                onClick={() => onManageProduits(e)}
                className="w-full mt-3 py-2 rounded-xl text-xs font-bold"
                style={{ background: C.ink, color: C.white, fontFamily: FONT_DISPLAY }}
              >
                Voir le stock
              </button>
            )}
...
function EcommercantProduitsPanel({ ecommercant, produits, onBack, onAdd, onEdit }) {
  return (
    <div>
      <button onClick={onBack} className="text-xs font-semibold underline mb-3" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
        ← Retour
      </button>
      <p className="text-sm font-bold mb-3" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>
        Stock de {ecommercant.ecommercant_boutique_name || ecommercant.name}
      </p>
      <button
        onClick={onAdd}
        className="w-full flex items-center justify-center gap-1.5 mb-3 py-2.5 rounded-xl"
        style={{ border: `1px dashed ${C.line}` }}
      >
        <Plus size={13} color={C.inkSoft} />
        <span className="text-xs font-semibold" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>Ajouter un produit</span>
      </button>
      {produits.length === 0 ? (
        <EmptyState icon={Package} title="Aucun produit" sub="Ajoutez le premier produit de cet e-commerçant." />
      ) : (
        <div className="space-y-3">
          {produits.map((p) => (
            <button key={p.id} onClick={() => onEdit(p)} className="w-full text-left">
              <StockProduitCard produit={p} />
            </button>
          ))}
        </div>
      )}
...
function StockPricingRow({ pricing, onSave }) {
  const [fraisFixeMois, setFraisFixeMois] = useState(String(pricing.frais_fixe_mois));
  const [fraisPourcentValeur, setFraisPourcentValeur] = useState(String(Number(pricing.frais_pourcent_valeur) * 100));
  const [saving, setSaving] = useState(false);

  const save = async () => {
    setSaving(true);
    try {
      await onSave(pricing.categorie, {
        fraisFixeMois: Number(fraisFixeMois),
        fraisPourcentValeur: Number(fraisPourcentValeur) / 100,
        exempleProduits: pricing.exemple_produits,
      });
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="rounded-2xl p-4" style={{ background: C.white, border: `1px solid ${C.line}` }}>
      <p className="text-sm font-bold capitalize" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>{pricing.categorie}</p>
      <p className="text-[11px] mt-0.5" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>{pricing.exemple_produits}</p>
      <div className="grid grid-cols-2 gap-2 mt-3">
        <TextField label="Frais fixe/mois" value={fraisFixeMois} onChange={setFraisFixeMois} />
        <TextField label="% valeur/mois" value={fraisPourcentValeur} onChange={setFraisPourcentValeur} />
      </div>
      <button onClick={save} disabled={saving} className="w-full mt-3 py-2 rounded-xl text-xs font-bold" style={{ background: C.ink, color: C.white, fontFamily: FONT_DISPLAY }}>
        {saving ? "Enregistrement..." : "Enregistrer"}
      </button>
    </div>
  );
...
function StockFraisRow({ frais, onMarkPaid }) {
  const [note, setNote] = useState("");
  const [saving, setSaving] = useState(false);
  const solde = Number(frais.montant_du) - Number(frais.acompte_verse || 0);

  const confirmPaid = async () => {
    setSaving(true);
    try {
      await onMarkPaid(frais.id, note);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="rounded-2xl p-4" style={{ background: C.white, border: `1px solid ${frais.paye ? C.line : C.zem}` }}>
      <div className="flex items-center justify-between mb-2">
        <Tag tone={frais.paye ? "lagoon" : "zem"}>{frais.paye ? "Payé" : "Non payé"}</Tag>
        <span className="text-[10px]" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>{frais.mois}</span>
      </div>
      <p className="text-sm font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>{frais.ecommercantName}</p>
      <p className="text-xs mt-1" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>{frais.ecommercantPhone}</p>
      <div className="flex justify-between mt-3">
        <p className="text-xs" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>Montant dû</p>
        <p className="text-xs font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>{Number(frais.montant_du).toLocaleString()} FCFA</p>
      </div>
      {Number(frais.acompte_verse) > 0 && (
        <div className="flex justify-between mt-1">
          <p className="text-xs" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>Acompte versé</p>
          <p className="text-xs" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>{Number(frais.acompte_verse).toLocaleString()} FCFA</p>
        </div>
      )}
      <div className="flex justify-between mt-1">
        <p className="text-xs font-bold" style={{ color: C.ink, fontFamily: FONT_BODY }}>Solde</p>
        <p className="text-xs font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>{solde.toLocaleString()} FCFA</p>
      </div>
      {!frais.paye && (
        <div className="mt-3">
          <TextField label="Note (optionnel)" value={note} onChange={setNote} placeholder="Ex : réglé par MoMo le 05/07" />
          <button onClick={confirmPaid} disabled={saving} className="w-full mt-2 py-2 rounded-xl text-xs font-bold" style={{ background: C.lagoon, color: C.white, fontFamily: FONT_DISPLAY }}>
            {saving ? "Enregistrement..." : "Marquer comme payé"}
          </button>
        </div>
      )}
      {frais.note && frais.paye && (
        <p className="text-[11px] mt-2 italic" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>Note : {frais.note}</p>
      )}
    </div>
  );
}

function EntrepotConfigCard({ entrepot, onSave }) {
  const [label, setLabel] = useState(entrepot?.label || "Entrepôt Manhia");
  const [lat, setLat] = useState(entrepot?.lat != null ? String(entrepot.lat) : "");
  const [lng, setLng] = useState(entrepot?.lng != null ? String(entrepot.lng) : "");
  const [pickerOpen, setPickerOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  ...
  const save = async () => {
    setSaving(true);
    try {
      await onSave({ label, lat: lat ? Number(lat) : null, lng: lng ? Number(lng) : null, country: entrepot?.country || "benin" });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="rounded-2xl p-4" style={{ background: C.white, border: `1px solid ${C.line}` }}>
      <p className="text-sm font-bold mb-3" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>Adresse de l'entrepôt</p>
      <TextField label="Nom du lieu" value={label} onChange={setLabel} />
      <button
        onClick={() => setPickerOpen(true)}
        className="w-full text-left rounded-xl px-3.5 py-3 mt-3"
        style={{ background: C.sand, border: `1px solid ${C.line}` }}
      >
        <p className="text-[10px] font-semibold uppercase tracking-wide" style={{ color: C.inkSoft, fontFamily: FONT_DISPLAY }}>Position</p>
        <p className="text-sm" style={{ color: lat && lng ? C.ink : C.inkSoft, fontFamily: FONT_BODY }}>
          {lat && lng ? `${Number(lat).toFixed(5)}, ${Number(lng).toFixed(5)}` : "Toucher pour choisir sur la carte"}
        </p>
      </button>
      <button onClick={save} disabled={saving || !lat || !lng} className="w-full mt-3 py-2.5 rounded-xl text-xs font-bold" style={{ background: C.ink, color: C.white, fontFamily: FONT_DISPLAY, opacity: !lat || !lng ? 0.5 : 1 }}>
        {saving ? "Enregistrement..." : "Enregistrer l'entrepôt"}
      </button>
      ...
      {pickerOpen && (
        <LocationPickerModal
          label="Entrepôt"
          color="depart"
          initialPosition={lat && lng ? { lat: Number(lat), lng: Number(lng), label } : null}
          country={entrepot?.country || "benin"}
          favorites={[]}
          onSaveFavorite={() => {}}
          onClose={() => setPickerOpen(false)}
          onConfirm={(place) => {
            ...
            if (place.label) setLabel(place.label);
            setPickerOpen(false);
          }}
        />
      )}
...
// Regroupe la configuration de l'entrepôt et la grille tarifaire de stockage par catégorie.
// Utilisé comme sous-onglet "Réglages" dans StockAdminTab.
function StockConfigPanel({ entrepot, pricingStockage, onSaveEntrepot, onSavePricing }) {
  return (
    <div className="space-y-5">
      <EntrepotConfigCard entrepot={entrepot} onSave={onSaveEntrepot} />
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.14em] mb-3" style={{ color: C.inkSoft, fontFamily: FONT_DISPLAY }}>
          Tarification du stockage
        </p>
        <div className="space-y-3">
          {pricingStockage.map((p) => (
            <StockPricingRow key={p.categorie} pricing={p} onSave={onSavePricing} />
          ))}
        </div>
      </div>
    </div>
...
// Regroupe le sélecteur de mois, la génération des frais, et la liste des lignes de frais
// dus/payés par e-commerçant. Utilisé comme sous-onglet "Frais & paiements" dans StockAdminTab.
function StockFraisPanel({ moisSelectionne, onChangeMois, fraisDuMois, onGenerer, onMarkPaid }) {
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        <input
          ...
          onChange={(e) => onChangeMois(e.target.value)}
          className="rounded-xl px-3.5 py-2.5 text-sm outline-none flex-1"
          style={{ background: C.white, border: `1px solid ${C.line}`, color: C.ink, fontFamily: FONT_BODY }}
        />
        <button onClick={onGenerer} className="px-3.5 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap" style={{ background: C.ink, color: C.white, fontFamily: FONT_DISPLAY }}>
          Générer les frais
        </button>
      </div>
      {fraisDuMois.length === 0 ? (
        <EmptyState icon={Wallet} title="Aucun frais généré" sub="Appuyez sur « Générer les frais » pour calculer les montants dus ce mois-ci." />
      ) : (
        <div className="space-y-3">
          {fraisDuMois.map((f) => (
            <StockFraisRow key={f.id} frais={f} onMarkPaid={onMarkPaid} />
          ))}
        </div>
      )}
    </div>
...
// Vue globale de l'inventaire : tous les produits de tous les e-commerçants, avec la valeur
// du stock disponible et une estimation du frais de stockage dû ce mois-ci pour chaque produit.
// Permet à l'admin de voir l'ensemble du stock en un coup d'œil, sans filtrer par e-commerçant.
function StockInventairePanel({ produits, pricingStockage, moisSelectionne, onEdit }) {
  const [filtreStatut, setFiltreStatut] = useState("tous");

  const pricingParCategorie = {};
  pricingStockage.forEach((p) => { pricingParCategorie[p.categorie] = p; });

  const produitsFiltres = filtreStatut === "tous" ? produits : produits.filter((p) => p.statut === filtreStatut);

  const valeurTotale = produits.reduce((s, p) => s + p.quantiteDisponible * p.valeurUnitaire, 0);
  const { total: fraisTotalEstime } = calculerFraisStockageMensuel(produits, pricingStockage, moisSelectionne);

  return (
    <div>
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="rounded-2xl p-4" style={{ background: C.lagoonDeep }}>
          <p className="text-[11px]" style={{ color: "#BFE0D6", fontFamily: FONT_BODY }}>Valeur totale du stock</p>
          <p className="text-lg font-bold mt-1" style={{ color: C.white, fontFamily: FONT_DISPLAY }}>
            {valeurTotale.toLocaleString()} FCFA
          </p>
        </div>
        <div className="rounded-2xl p-4" style={{ background: C.ink }}>
          <p className="text-[11px]" style={{ color: "#B8B2A3", fontFamily: FONT_BODY }}>Frais estimés ({moisSelectionne})</p>
          <p className="text-lg font-bold mt-1" style={{ color: C.white, fontFamily: FONT_DISPLAY }}>
            {fraisTotalEstime.toLocaleString()} FCFA
          </p>
        </div>
      </div>

      <div className="flex gap-2 mb-4 flex-wrap">
        {["tous", "disponible", "a_verifier", "epuise"].map((s) => (
          <button
            key={s}
            onClick={() => setFiltreStatut(s)}
            className="px-3 py-1.5 rounded-full text-[11px] font-bold capitalize"
            style={{ background: filtreStatut === s ? C.ink : C.white, color: filtreStatut === s ? C.white : C.ink, border: `1px solid ${C.line}`, fontFamily: FONT_DISPLAY }}
          >
            {s === "tous" ? "Tous" : s === "a_verifier" ? "À vérifier" : s === "epuise" ? "Épuisé" : "Disponible"}
          </button>
        ))}
      </div>

      {produitsFiltres.length === 0 ? (
        <EmptyState icon={Package} title="Aucun produit" sub="Aucun produit ne correspond à ce filtre." />
      ) : (
        <div className="space-y-3">
          {produitsFiltres.map((p) => {
            const pricing = pricingParCategorie[p.categorie];
            const jours = pricing ? joursStockageDansLeMois(p, moisSelectionne) : 0;
            return (
              <button key={p.id} onClick={() => onEdit && onEdit(p)} className="w-full text-left">
                <div className="rounded-2xl p-4" style={{ background: C.white, border: `1px solid ${C.line}` }}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[11px] font-bold" style={{ color: C.lagoon, fontFamily: FONT_DISPLAY }}>{p.ecommercantName}</span>
                    <span className="text-[10px] uppercase" style={{ color: C.inkSoft, fontFamily: FONT_DISPLAY }}>{p.categorie}</span>
                  </div>
                  <StockProduitCard produit={p} />
                  <p className="text-[11px] mt-2" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
                    {jours > 0 ? `Stocké depuis ${jours} jour(s) ce mois-ci` : "Non stocké ce mois-ci"}
                  </p>
                </div>
              </button>
            );
...
// Panneau Admin principal du module Stock : interrupteur global, demandes e-commerçants,
// gestion des produits par e-commerçant, entrepôt, tarification, et suivi des frais mensuels.
function StockAdminTab() {
  const [stockEnabled, setStockEnabled] = useState(false);
  const [toggling, setToggling] = useState(false);
  const [subTab, setSubTab] = useState("demandes");
  const [ecommercants, setEcommercants] = useState([]);
  const [entrepot, setEntrepot] = useState(null);
  const [pricingStockage, setPricingStockage] = useState([]);
  const [selectedEcommercant, setSelectedEcommercant] = useState(null);
  const [produitsSelected, setProduitsSelected] = useState([]);
  const [addingProduit, setAddingProduit] = useState(false);
  const [editProduitTarget, setEditProduitTarget] = useState(null);
  const [moisSelectionne, setMoisSelectionne] = useState(moisKey());
  const [fraisDuMois, setFraisDuMois] = useState([]);
  const [tousLesProduits, setTousLesProduits] = useState([]);
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    const [enabled, ec, ent, ps] = await Promise.all([
      isStockModuleEnabled(),
      fetchAllEcommercants(),
      fetchEntrepotConfig(),
      fetchPricingStockage(),
    ]);
    setStockEnabled(enabled);
    setEcommercants(ec);
    setEntrepot(ent);
    setPricingStockage(ps);
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    if (subTab === "frais") {
      fetchAllFraisStockageForMonth(moisSelectionne).then(setFraisDuMois);
    }
    if (subTab === "inventaire") {
      fetchAllStockProduits().then(setTousLesProduits);
    }
  }, [subTab, moisSelectionne]);

  const handleToggle = async () => {
    setToggling(true);
    try {
      await setStockModuleEnabled(!stockEnabled);
      setStockEnabled((v) => !v);
    } finally {
      setToggling(false);
    }
  ...
  const openProduits = async (ecommercant) => {
    setSelectedEcommercant(ecommercant);
    const p = await fetchStockProduitsForEcommercant(ecommercant.id);
    setProduitsSelected(p);
  };

  const reloadProduitsSelected = async () => {
    if (!selectedEcommercant) return;
    const p = await fetchStockProduitsForEcommercant(selectedEcommercant.id);
    setProduitsSelected(p);
  };

  // Génère (ou régénère) le montant dû du mois pour tous les e-commerçants vérifiés,
  // à partir de leur stock actuel et de la grille tarifaire.
  const genererFraisDuMois = async () => {
    const verifies = ecommercants.filter((e) => e.ecommercant_verification_status === "verifie");
    for (const e of verifies) {
      const produits = await fetchStockProduitsForEcommercant(e.id);
      if (produits.length === 0) continue;
      const { total } = calculerFraisStockageMensuel(produits, pricingStockage, moisSelectionne);
      if (total > 0) {
        await upsertFraisStockage(e.id, moisSelectionne, { montantDu: total, acompteVerse: 0, paye: false });
      }
    }
    fetchAllFraisStockageForMonth(moisSelectionne).then(setFraisDuMois);
  };
  ...
  return (
    <div>
      <div className="rounded-2xl p-4 mb-5" style={{ background: stockEnabled ? C.lagoonDeep : C.white, border: stockEnabled ? "none" : `1px solid ${C.line}` }}>
        <div className="flex items-center justify-between">
          <div className="flex-1 pr-3">
            <p className="text-sm font-bold" style={{ color: stockEnabled ? C.white : C.ink, fontFamily: FONT_DISPLAY }}>
              Module Stock {stockEnabled ? "activé" : "désactivé"}
            </p>
            <p className="text-xs mt-1 leading-snug" style={{ color: stockEnabled ? "#BFE0D6" : C.inkSoft, fontFamily: FONT_BODY }}>
              {stockEnabled
                ? "Les clients peuvent demander à devenir e-commerçant. Le bouton apparaît dans leur profil."
                : "Le module reste invisible pour les clients tant qu'il n'est pas activé."}
            </p>
          </div>
          <button
            onClick={handleToggle}
            disabled={toggling}
            className="w-14 h-8 rounded-full flex items-center px-1 shrink-0"
            style={{ background: stockEnabled ? C.zem : C.sandDeep, justifyContent: stockEnabled ? "flex-end" : "flex-start" }}
          >
            <div className="w-6 h-6 rounded-full" style={{ background: C.white }} />
          </button>
        </div>
      ...
      {loading ? (
        ...
        <>
          <div className="flex gap-2 mb-4 flex-wrap">
            <button onClick={() => { setSubTab("demandes"); setSelectedEcommercant(null); }} className="px-3 py-2 rounded-full text-xs font-bold" style={{ background: subTab === "demandes" ? C.ink : C.white, color: subTab === "demandes" ? C.white : C.ink, border: `1px solid ${C.line}`, fontFamily: FONT_DISPLAY }}>
              E-commerçants
            </button>
            <button onClick={() => { setSubTab("inventaire"); setSelectedEcommercant(null); }} className="px-3 py-2 rounded-full text-xs font-bold" style={{ background: subTab === "inventaire" ? C.ink : C.white, color: subTab === "inventaire" ? C.white : C.ink, border: `1px solid ${C.line}`, fontFamily: FONT_DISPLAY }}>
              Inventaire
            </button>
            <button onClick={() => { setSubTab("frais"); setSelectedEcommercant(null); }} className="px-3 py-2 rounded-full text-xs font-bold" style={{ background: subTab === "frais" ? C.ink : C.white, color: subTab === "frais" ? C.white : C.ink, border: `1px solid ${C.line}`, fontFamily: FONT_DISPLAY }}>
              Frais & paiements
            </button>
            <button onClick={() => { setSubTab("reglages"); setSelectedEcommercant(null); }} className="px-3 py-2 rounded-full text-xs font-bold" style={{ background: subTab === "reglages" ? C.ink : C.white, color: subTab === "reglages" ? C.white : C.ink, border: `1px solid ${C.line}`, fontFamily: FONT_DISPLAY }}>
              Réglages
            </button>
          </div>

          {subTab === "demandes" && !selectedEcommercant && (
            <EcommercantsVerificationPanel
              ecommercants={ecommercants}
              onValidate={async (e) => { await setEcommercantVerificationStatus(e.id, "verifie"); load(); }}
              onRefuse={async (e) => { await setEcommercantVerificationStatus(e.id, "refuse"); load(); }}
              onManageProduits={openProduits}
            />
          )}

          {subTab === "demandes" && selectedEcommercant && (
            <EcommercantProduitsPanel
              ecommercant={selectedEcommercant}
              produits={produitsSelected}
              onBack={() => setSelectedEcommercant(null)}
              onAdd={() => setAddingProduit(true)}
              onEdit={setEditProduitTarget}
            />
          )}

          {subTab === "inventaire" && (
            <StockInventairePanel
              produits={tousLesProduits}
              pricingStockage={pricingStockage}
          ...
          {subTab === "frais" && (
            <StockFraisPanel
              ...
              fraisDuMois={fraisDuMois}
              onGenerer={genererFraisDuMois}
              onMarkPaid={async (id, note) => {
                await markFraisStockagePaye(id, note);
                fetchAllFraisStockageForMonth(moisSelectionne).then(setFraisDuMois);
              }}
            />
          )}
          ...
          {subTab === "reglages" && (
            <StockConfigPanel
              entrepot={entrepot}
              pricingStockage={pricingStockage}
              onSaveEntrepot={async (patch) => { await updateEntrepotConfig(patch); load(); }}
              onSavePricing={async (categorie, patch) => { await updatePricingStockage(categorie, patch); load(); }}
            />
          )}
        </>
      )}
      ...
      {addingProduit && (
        <AddStockProduitModal
          ecommercants={[selectedEcommercant]}
          onClose={() => setAddingProduit(false)}
          onDone={() => { setAddingProduit(false); reloadProduitsSelected(); }}
        />
      )}
      {editProduitTarget && (
        <EditStockProduitModal
          produit={editProduitTarget}
          onClose={() => setEditProduitTarget(null)}
          onDone={() => {
            setEditProduitTarget(null);
            reloadProduitsSelected();
            if (subTab === "inventaire") fetchAllStockProduits().then(setTousLesProduits);
          }}
        />
      )}
    </div>
...
function AdminPage({ currentUser, onAdminAuth }) {
  ...
  useEffect(() => {
    if (isAdminAuthenticated(currentUser)) load();
  }, [currentUser, load]);

  if (!isAdminAuthenticated(currentUser)) {
    return <AdminLogin onSuccess={() => { /* onAuth (session Supabase) déclenche déjà le rechargement du profil */ }} />;
  }
  ...
  return (
    <div style={{ background: C.sand, minHeight: "100vh" }}>
      <div className="px-5 pt-6 pb-4 flex items-center justify-between" style={{ background: C.ink }}>
        ...
        <button
          onClick={async () => { await adminSignOut(); window.location.hash = ""; }}
...
export default function ManhiaPrototype() {
  ...
  // isResetRoute est piloté par l'événement PASSWORD_RECOVERY de Supabase (voir plus
  // bas dans onAuthStateChange), pas par une lecture du hash au premier rendu.
  // Raison : le SDK Supabase lit et NETTOIE automatiquement le hash
  // (#access_token=...&type=recovery...) dès qu'il détecte une session dans l'URL,
  // parfois avant même que ce useState ne s'exécute. Dépendre du hash créait donc
  // une race condition : selon le timing, isResetRoute pouvait rester à false alors
  // qu'on était bien dans un flux de récupération de mot de passe, et l'utilisateur
  // atterrissait connecté directement sur son tableau de bord au lieu de voir
  // l'écran "Nouveau mot de passe". L'événement PASSWORD_RECOVERY, lui, est toujours
  // émis de façon fiable par le SDK, quel que soit le moment où le hash est lu.
  ...
  const loadProfileFromSession = useCallback(async (session) => {
    ...
    if (profile) {
      setUser({
        ...
        ecommercantVerificationStatus: profile.ecommercant_verification_status || "non_demande",
  ...
  useEffect(() => {
    ...
    // et récupération de mot de passe). L'événement PASSWORD_RECOVERY est émis par Supabase
    // dès qu'il détecte et consomme un lien de reset valide dans l'URL — c'est le signal
    // fiable pour afficher l'écran "Nouveau mot de passe" (voir isResetRoute ci-dessus).
    ...
    return (
      <div className="min-h-screen" style={{ background: C.sand }}>
        <div className="max-w-md mx-auto shadow-2xl min-h-screen" style={{ background: C.sand }}>
          <ResetPasswordScreen onDone={() => { window.location.hash = ""; setIsResetRoute(false); }} />
        </div>
      </div>
    );
  }
  ...
  if (isAdminRoute) {
    return (
      <div className="min-h-screen" style={{ background: C.sand }}>
        <div className="max-w-md mx-auto shadow-2xl min-h-screen" style={{ background: C.sand }}>
          <AdminPage currentUser={user} onAdminAuth={handleAuth} />
        </div>
      </div>
    );
  }
