  ...
  return (
    <div className="pb-28">
      ...
      <div className="px-5 mt-5">
        ...
        {payerType === "client" && (
          <div className="mt-3">
            <p className="text-[11px] font-bold uppercase tracking-[0.1em] mb-2" style={{ color: C.inkSoft, fontFamily: FONT_DISPLAY }}>
              Quand souhaitez-vous payer ?
            </p>
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => setPaymentTiming("collecte")}
                className="rounded-xl p-3 text-left"
                style={{ background: C.white, border: `2px solid ${paymentTiming === "collecte" ? C.lagoon : C.line}` }}
              >
                <p className="text-xs font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>À la collecte</p>
                <p className="text-[10px] mt-0.5 leading-snug" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
                  Je paie le livreur dès qu'il vient récupérer le colis
                </p>
              </button>
              <button
                onClick={() => setPaymentTiming("livraison")}
                className="rounded-xl p-3 text-left"
                style={{ background: C.white, border: `2px solid ${paymentTiming === "livraison" ? C.lagoon : C.line}` }}
              >
                <p className="text-xs font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>À la livraison</p>
                <p className="text-[10px] mt-0.5 leading-snug" style={{ color: C.inkSoft, fontFamily: FONT_BODY }}>
                  Je paie une fois le colis arrivé à destination
                </p>
              </button>
            </div>
          </div>
...
function AdminPage() {
  ...
  const load = useCallback(async () => {
    const [c, p, l, u] = await Promise.all([
      fetchComplaints(),
...
function PaymentScreen({ courseId }) {
  ...
  const pay = async () => {
    ...
    try {
      ...
      await updateCourse(course.id, {
        paid: true,
        paymentRef: result.transaction?.id ? String(result.transaction.id) : null,
