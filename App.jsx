...
function CourseCard({ course, highlight, onCancel, onReport, onOpenReceipt }) {
  const statusTone = {
    en_attente: "zem",
    acceptee: "lagoon",
...
function ReceiptScreen({ course: initialCourse, onClose }) {
...
function ClientApp({ user, onLogout }) {
  ...
  return (
    <div style={{ background: C.sand, minHeight: "100vh" }} className="flex flex-col">
          ...
          <div className="pb-6">
            ...
            <div className="px-5 mt-4 space-y-3">
              {courses.length === 0 ? (
                ...
                courses.map((c) => (
                  <CourseCard key={c.id} course={c} onCancel={setCancelTarget} onReport={setReportTarget} onOpenReceipt={setReceiptTarget} />
                ))
              )}
            </div>
          </div>
      ...
      {receiptTarget && (
        <ReceiptScreen course={receiptTarget} onClose={() => setReceiptTarget(null)} />
      )}
    </div>
  );
}
                  ...
                  <div key={c.id} className="rounded-xl p-3" style={{ background: C.white, border: `1px solid ${C.line}` }}>
                    ...
                    {c.clientRating ? (
                      ...
                      <button
                        ...
                        className="text-[11px] font-semibold underline mt-2"
...
function AdminPage() {
  ...
  const load = useCallback(async () => {
    const [c, p, l, u, s] = await Promise.all([
      fetchComplaints(),
