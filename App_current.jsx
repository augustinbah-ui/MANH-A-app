...
async function fetchAllAdsForAdmin() {
  const { data, error } = await supabase
    .from("ads")
    .select("*")
    .order("display_order", { ascending: true });
...
async function createAd({ imageUrl, linkUrl, title, displayOrder }) {
...
async function updateAd(id, patch) {
  ...
  const { error } = await supabase.from("ads").update(dbPatch).eq("id", id);
  if (error) throw error;
}
...
function AuthScreen({ onAuth }) {
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signup") {
        await signUpUser({ name, phone, email, password, role });
        setLoading(false);
        setSignupDone(true);
      } else {
...
function AdsManagementPanel({ ads, onAdd, onEditTarget, onDelete, onToggleActive }) {
  return (
    <div>
      <button
        onClick={onAdd}
...
function AdEditModal({ ad, onClose, onDone }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (ad) {
        await updateAd(ad.id, { imageUrl: finalImageUrl, linkUrl, title, displayOrder: orderNum });
      } else {
        await createAd({ imageUrl: finalImageUrl, linkUrl, title, displayOrder: orderNum });
      }
      onDone();
    } catch {
      setError("Erreur lors de l'enregistrement. Réessayez.");
...
function AdminPage() {
  ...
  const load = useCallback(async () => {
    const [c, p, l, u, s, au, ad] = await Promise.all([
      fetchComplaints(),
      ...
      <div className="px-5 mt-4 pb-10 space-y-3">
        {loading ? (
          ...
          <AdsManagementPanel
            ...
            onDelete={async (ad) => { await deleteAd(ad.id); load(); }}
            onToggleActive={async (ad) => { await updateAd(ad.id, { active: !ad.active }); load(); }}
          />
        )}
      </div>
...
function PaymentScreen({ courseId }) {
  ...
  const pay = async () => {
    ...
    try {
      ...
      // Argent prélevé à ce stade : la seule chose qui peut encore échouer est la
      // synchronisation avec Supabase, jamais le paiement lui-même.
      await confirmPaidStatus(result.transaction?.id);
    } catch {
