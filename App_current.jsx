  ...
  return (
    <div>
      ...
      {codes.length === 0 ? (
        ...
        <div className="space-y-3">
          {codes.map((c) => (
            <div key={c.id} className="rounded-2xl p-4" style={{ background: C.white, border: `1px solid ${c.active ? C.line : C.line}` }}>
              <div className="flex items-center justify-between mb-2">
                <Tag tone={c.active ? "lagoon" : "ink"}>{c.active ? "Actif" : "Inactif"}</Tag>
                <span className="text-lg font-bold" style={{ color: C.ink, fontFamily: FONT_DISPLAY }}>{c.code}</span>
              </div>
              <div className="flex justify-between text-xs" style={{ fontFamily: FONT_BODY }}>
                <span style={{ color: C.inkSoft }}>Réduction max : {Number(c.max_discount).toLocaleString()} FCFA</span>
                <span style={{ color: C.inkSoft }}>Max {c.max_uses_per_user}× par client</span>
              </div>
              <div className="flex gap-2 mt-3">
                <button
...
function AdminPage() {
  ...
  const load = useCallback(async () => {
    const [c, p, l, u, s, au, ad, bl, pc] = await Promise.all([
      fetchComplaints(),
