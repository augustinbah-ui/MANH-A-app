  ...
  const confirmBooking = async () => {
    ...
    try {
      ...
      const created = await createCourse({
        ...
        history: [{ label: appliedPromo ? `Course créée (code promo ${appliedPromo.promo.code} appliqué)` : "Course créée", at: Date.now() }],
      });
      if (appliedPromo) {
...
function AdminPage() {
  ...
  const load = useCallback(async () => {
    const [c, p, l, u, s, au, ad, bl] = await Promise.all([
      fetchComplaints(),
